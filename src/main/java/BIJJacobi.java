 ...
 * Redesigned to be faster, for double. Internal calculations are still double though.
...
public class BIJJacobi {
    ...
    protected final static double UNDERFLOW_LIMIT = (double) 0.0000000000001; // Value used for threshold of underflow.
    /**
     ...
     */
    public BIJJacobi(double[][] m, boolean doFeedback) {
        n = m.length;
        if (m.length != m[0].length)
            throw new IllegalArgumentException("BIJJacobi: not square matrix");
        a = new double[n][n];  // matrix of which you want the E-values.
        // Save the matrix m.
    ...
    public void compute() throws Exception {
        ...
        for (int ip = 0; ip < n; ip++) {
            for (int iq = 0; iq < n; iq++)
                eigenvectors[ip][iq] = 0.0d;   // v[ip][iq] = 0
            eigenvectors[ip][ip] = 1.0d; // make v an identity matrix.
        }
        ...
        for (int i = 0; i < 50; i++) {
            ...
            if (sm < zero) {
                sweeps = i;
                //System.out.println("Trovato sm < zero - iter: " + sweeps + " nrot: " + nrot);
                return;
            }
           /* if (sm == 0.0) {
                sweeps = i;
            ...
            else thresh = 0.0d; /* the rest of the sweeps */
            for (int ip = 0; ip < (n - 1); ip++) {
                for (int iq = ip + 1; iq < n; iq++) {
                    // g is class variable.
                    ...
                    if (i > 3 && (Math.abs(eigenvalues[ip]) + g) == Math.abs(eigenvalues[ip])
                        ...
                        double c = 1d / (double) Math.sqrt(1d + t * t);
                        double s = t * c;
                        double tau = s / (1d + c);
                        h = t * a[ip][iq];
                        z[ip] -= h;
        ...
        throw new Exception("Too many iterations in BIJJacobi");
    }
     ...
     */
    protected void rotate(double a[][], int i, int j, int k, int l, double s, double tau) {
        // modifies class variables g and h.
        g = a[i][j];
        h = a[k][l];
        a[i][j] = (double) (g - s * (h + g * tau));
        a[k][l] = (double) (h + s * (g - h * tau));
    }
